...
function Nh3No2Field() {
    ...
    const fetchPondOptions = useCallback(() => {
        ...
        callApi(
            [DashboardRequestApi.pondRequest.getPondRequestByPondTypeIdAndFarmId(formData.pondTypeId, farmId)],
            (res) => {
                const ponds = res[0] || [];
                setPondOptions(
                    ponds.map((pond) => ({
        ...
        (e) => {
            ...
            callApi(
                ...
                () => {
                    ...
                    setFormData({ pondTypeId: '', pondId: '', name: '', value: '', timestamp: '' });
                    setErrorMessage('');
                },
                (err) => {
                    setIsLoading(false);
    ...
    return (
        <div className="bg-white p-4 rounded-lg">
            <form onSubmit={handleSubmit} className="space-y-3">
                {/* Chọn loại ao */}
                <div>
                    <label className="block text-teal-800 font-medium mb-1" htmlFor="pondTypeId">
                        Chọn loại ao
                    </label>
                    <select
                        id="pondTypeId"
